...
class TaskViewModel(private val taskRepository: TaskRepository) : ViewModel() {
    ...
    fun getAllTask() {
        viewModelScope.launch(Dispatchers.IO) {
            listAllTaskM.postValue(taskRepository.getAllTaskOfProject(1).map {
                TaskUI(it.id, it.title, it.tag, it.description, it.state, it.deadline)
            }
            )
...
//        ))
    ...
    fun getAllTaskOfProject(projectID: Int) {
        viewModelScope.launch(Dispatchers.IO) {
            listDisplayM.postValue(taskRepository.getAllTaskOfProject(1).map {
                TaskUI(it.id, it.title, it.tag, it.description, it.state, it.deadline)
            })
